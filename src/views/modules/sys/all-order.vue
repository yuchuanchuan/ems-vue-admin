<template>
  <div class="all-order">
    <el-form :inline="true" :model="dataAllForm">
      ...
      <el-form-item>
        <el-input v-model="dataAllForm.mailNum" placeholder="邮件号" clearable />
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item>
        <el-button v-if="type===1" type="primary" @click="printDetails">批量打印详情单</el-button>
      </el-form-item>
    </el-form>
    <el-table
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
        <template slot-scope="scope">
          <el-button
            v-if="isAuth('sys:order:update') && (scope.row.status === 1 || scope.row.status === 2 || scope.row.status === 6 || scope.row.status === 7 || scope.row.status === 8 || scope.row.status === 9)"
            type="text"
            size="small"
            @click="addOrUpdateHandle(scope.row.orderId)">修改</el-button>
          <el-button v-if="isAuth('sys:order:info')" type="text" size="small" @click="viewOrder(scope.row.orderId)">查看</el-button>
          <el-button
            v-if="(type===3||type===2) && (scope.row.status === 2 || scope.row.status === 6 || scope.row.status === 7 || scope.row.status === 8 || scope.row.status === 9)"
            type="text"
            size="small"
            @click="cancelOrderApply(scope.row.orderId)">退款申请</el-button>
          <el-button
            v-if="isAuth('sys:order:delete')&&(type===1) && (scope.row.status === 14)"
            type="text"
            size="small"
            @click="cancelOrderStatus(scope.row.orderId, scope.row.openid,scope.row.cancelReason)">确认退款</el-button>
          <el-button
            v-if="isAuth('sys:order:delete')&&(type===1) && (scope.row.status === 2 || scope.row.status === 6 || scope.row.status === 7 || scope.row.status === 8 || scope.row.status === 9)"
            type="text"
            size="small"
            @click="cancelOrderStatus(scope.row.orderId, scope.row.openid,'')">退款</el-button>
          ...
          <el-button v-if="isAuth('sys:order:delete')&&(type===1)" type="text" size="small" @click="viewPrintDetail(scope.row.orderId,'single')">详情单打印</el-button>
          <el-button
            v-if="isAuth('sys:order:delete') && (scope.row.status !== 3 && scope.row.status !== 13)"
            type="text"
            size="small"
            disabled
    ...
    <!-- 打印详情单 -->
    <PrintDetail v-if="printDetailVisible" ref="PrintDetail" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getAllDataList(page) {
      ...
      this.$http({
        ...
        params: this.$http.adornParams({
          ...
          'mailNum': this.dataAllForm.mailNum,
    ...
    // 查看详情单
    viewPrintDetail(id) {
      this.printDetailVisible = true
      this.$nextTick(() => {
        this.$refs.PrintDetail.init(id)
      })
    },
    exportExcel() {
      if (this.createOrderTime && this.createOrderTime.length > 0) {
      ...
      this.$confirm(`是否打包下载证照图片?`, '提示', {
        ...
        this.downloadFile(this.$http.adornUrl('/sys/order/exportOrder') + '?startOrderTime=' +
                ...
                '&orderNumber=' + this.dataAllForm.orderNumber + '&mailNum=' + this.dataAllForm.mailNum + '&idCard=' + this.dataAllForm.idCard +
    ...
    printDetails() {
      ...
      this.printDetailVisible = true
      this.$nextTick(() => {
        this.$refs.PrintDetail.init(orderIdStr, 'multiple')
      })
    }
  }
}
