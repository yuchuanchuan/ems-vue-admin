  ...
  <div class="reward-order">
    <el-form :inline="true" :model="dataCancelForm">
      ...
      <el-form-item v-if="type == 1">
        <el-select v-model="dataCancelForm.bigAreaId" multiple placeholder="办理大区" width="100%" clearable @change="getAreaInfo">
          <el-option
      ...
      <el-form-item v-if="(type == 1  && dataCancelForm.bigAreaId != '') || type == 2">
        <el-select v-model="dataCancelForm.areaId" multiple placeholder="办理网点" width="100%" clearable>
          <el-option
  ...
  export default {
    ...
    methods: {
      ...
      getCancelDataList (page) {
        ...
        if(this.dataCancelForm.bigAreaId.length === 0){
          this.dataCancelForm.areaId = []
        }
        // 数据转换 areaId
        let multiAreaId = ''
        if(this.dataCancelForm.areaId && this.dataCancelForm.areaId.length > 0){
          multiAreaId = this.dataCancelForm.areaId.join(',')
        }
        // 数据转换，大区bigAreaId
        let multiBigAreaId = ''
        if(this.dataCancelForm.bigAreaId && this.dataCancelForm.bigAreaId.length > 0){
          multiBigAreaId = this.dataCancelForm.bigAreaId.join(',')
        }
      ...
      getAreaInfo(){
        ...
        this.$http({
          ...
          method: 'get',
          params: this.$http.adornParams({
            'areaId': this.type === 1 ? this.dataCancelForm.bigAreaId.join(',') : ''
          })
        }).then(({ data }) => {
          this.areaList = []
          if(data && data.code === 0){
